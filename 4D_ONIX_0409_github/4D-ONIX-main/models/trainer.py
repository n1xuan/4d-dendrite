...
class TrainModel(ABC):
    ...
    def set_input(self, input_data):
        ...
        if self.opt.use_time:
            self.all_times = (
                input_data[2].to(self.device, dtype=self.dtype).view(-1)
            )  # 2
    ...
    def generate_random_poses(self):
        ...
        for i in range(ProjAngles.shape[0]):
            world_mat.append(get_world_mat(theta_x, theta_y[i].item(), theta_z))
    ...
    def get_ray_function(self):
        ...
        if not self.random_proj:
            ...
            self._using_random_angles = False
        else:
            # 50%概率用固定角度（MSE监督有效），50%概率用随机角度（GAN监督有效）
            if np.random.rand() < 0.5:
                pose_target = self.all_poses[..., :3, :4].to(self.device)
                self._using_random_angles = False
            else:
                pose_target = self.generate_random_poses().unsqueeze(0)
                self._using_random_angles = True
    ...
    def get_ray_sample_grid(self):
        ...
        if self.with_phase:
            ...
            att_gt = self.sample_grid(self.all_images.squeeze(2), uv).to(self.device).reshape(-1)
    ...
    def optimization(self):
        ...
        if self.with_phase:
            ...
            # 1.6add 让后面的GAN能拿到数据手动赋值
            self.complex_coarse = self.atten_coarse
        ...
        # When using random angles, target_att is from fixed angles,
        # so MSE is invalid — zero it out, let GAN supervise instead
        if hasattr(self, '_using_random_angles') and self._using_random_angles:
            self.total_mse_loss = self.ray_loss * 0.0
        else:
            self.total_mse_loss = self.ray_loss * self.lambda_mse

        if self.opt.lambda_generator > 0:
            self.update_gan()
        else:
            # Pure MSE mode: skip GAN entirely
            self.total_mse_loss.backward()
            if self.clip_max != 0:
            ...
            self.optimizer.step()
            self.D_coarse_loss = torch.tensor(0.0)
            self.G_coarse_loss = torch.tensor(0.0)
    ...
    def update_gan(self):
        ...
        # reshape()改为self.opt.in_channel这样无论是1还是2都能跑
        x_fake = self.complex_coarse.reshape(self.opt.in_channel, -1).permute(1, 0)
