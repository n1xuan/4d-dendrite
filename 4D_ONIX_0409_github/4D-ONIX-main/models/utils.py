...
def data_preproc(images):
    ...
    images = images[:60]     # choose how many time stamps
...
def save_tensor_plot(data, save_folder=0, save_name=0):
    ...
    img_data = data.squeeze().cpu().numpy()
    plt.imshow(img_data, vmin=0, vmax=max(img_data.max(), 1e-6))
    plt.colorbar(shrink=0.6)
    plt.title(f"range: [{img_data.min():.4f}, {img_data.max():.4f}]")
    plt.axis("off")
